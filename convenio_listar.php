...
<html lang="en">
    <head>
		...
		<script type="text/javascript">
			function confirma(){
			   var press = confirm("Tem certeza que deseja excluir este Convênio ?");
				if(press){
                    ...
                    <div class="col-md-10 corpo">
                        <div class="page-header">
                            ...
                            <div class="col-md-2 link_tela">
                                <a href="convenio_cadastrar.php" class="btn btn-success btn-sm"><i class="fa fa-list-alt"></i>Novo</a>      
                            </div>
                        </div>
                        <div class="conteudo">
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    <tbody>
										 <?php
                                            $sql = "SELECT convenio, descricao from tbl_convenio where status_id = 1 order by descricao; ";

                                            $res = pg_query($con, $sql);
                                            for($i=0; $i<pg_num_rows($res); $i++){
                                                $convenio_id       = pg_fetch_result($res, $i, 'convenio'); 
                                                $descricao         = pg_fetch_result($res, $i, 'descricao'); 

                                                echo "<tr>";
                                                    echo "<td>".($i+1)."</td>";
                                                    echo "<td>$descricao</td>";
													echo "<td class='col-md-1'><a href='./convenio_cadastrar.php?convenio=$convenio_id' title='Editar'><i class='fa fa-pencil-square-o'></i></a></td>";
													echo "<td class='col-md-1'><a href='./convenio_cadastrar.php?excluir=$convenio_id' onclick='return confirma()' title='Excluir'><i class='fa fa-trash-o'></i></a></td>";
                                                echo "</tr>";
                                            }
                                         ?>
                                    </tbody>
